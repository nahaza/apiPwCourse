...
export class TokenHelper {
  ...
  static async getTokens(tokenPath: string = SPOTIFY_CONFIG.TOKEN_FILE_PATH): Promise<SpotifyTokenResponse> {
    ...
    if (!this.validateTokenStructure(tokens)) {
      throw new Error(`Invalid token structure in '${tokenPath}'. Please regenerate tokens.`);
    }
  ...
  static isTokenExpired(tokens: SpotifyTokenResponse): boolean {
    ...
    const expirationWithBuffer = tokens.expirationDate - SPOTIFY_CONFIG.TOKEN_BUFFER_TIME;
