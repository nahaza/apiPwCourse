...
function verifyResponseBodyDateBetween(responseBody: any, startDate: string, endDate: string) {
  ...
  expect(
    date >= start && date <= end,
    `Response body date ${responseBody.date} should be between ${startDate} and ${endDate}`
  ).toBe(true);
}
...
test(
  "NASA-APOD-003 get start date apod within random days - min 2 and max 11 days",
  { tag: ["@nasa", "@apod"] },
  async ({ request }) => {
    const minDays = 2,
      maxDays = 10;
...
test(
  "NASA-APOD-004 get end date apod within random days before - min 2 and max 5 days ",
  { tag: ["@smoke", "@nasa", "@apod"] },
  async ({ request }) => {
    const minDays = 2,
      maxDays = 5;
...
test("NASA-APOD-008 get no api key apod", { tag: ["@nasa", "@apod"] }, async ({ request }) => {
  ...
  expect(responseBody.error.message, "Response body error message").toBe(
